...
class CouplingLayer(torch.nn.Module):
    def __init__(self, input_dim, conditioning_size=None,
        ...
        self._s = torch.nn.Linear(self.out_dim, self.out_dim, bias=False)
        self._t = torch.nn.Linear(self.out_dim, self.out_dim, bias=False)
    ...
    def s(self, x, y):
        ...
        s = torch.tanh(s) * self.log_scale_factor.exp()
    ...
    def __init__(self, n_levels, levels_blocks, input_dim, hidden_size, n_hidden=1,
        ...
        if coupling_f is None:
            def coupling_f(ind, outd):
                ...
                s = [torch.nn.Linear(ind, hs), torch.nn.ReLU()]  # , torch.nn.Dropout(0.2)]
        ...
        self.register_parameter('std', None)

    def set_mean_std(self, mean, std):
        self.register_parameter('mean', torch.nn.Parameter(mean, requires_grad=False))
        self.register_parameter('std', torch.nn.Parameter(std, requires_grad=False))
        # self.mean = mean
        # self.std = std
