...
class PRERproj(NaiveMethod):
    ...
    def __init__(self, encoder: Encoder, config: ExperimentConfig, classes: int, device,
                 plot_dir: str,
        ...
        self.projector = torch.nn.Linear(encoder.flat_cnn_size, encoder.embedding_dim).to(self.device)
        ...
        self.ae_batch_size = ae_config.get('batch_size', config.train_config['batch_size'])
        ...
        decoder = Decoder(config.cl_config['dataset'], encoder, cond_size=self.ae_conditioner.size)
        ...
        self.nf_batch_size = nf.get('batch_size', config.train_config['batch_size'])
        ...
        if logger is not None:
            ...
            logger.info(F'\tNF parameters: {sum(p.numel() for p in self.get_generator().parameters())}')
    ...
    def rec_loss(self, x, xr, reduction='mean'):
        if self.rec_loss_type == 'mse':
            ...
            return reconstruction_loss(x, xr, reduction=reduction, softclip=self.softclip)
        if self.rec_loss_type == 'bce':
            return torch.nn.functional.binary_cross_entropy(x, xr, reduction=reduction)
        else:
            assert False
    ...
    def distance_function(self, e1, e2):
        if self.distance == 'cosine':
            ...
            return torch.sub(1.0, torch.nn.functional.cosine_similarity(e1, e2, dim=-1))
        ...
        if container.current_task.index > 0:
            with torch.no_grad():
                old_images, old_labels, _, classification_embeddings = \
                    self.get_sampled_images(self.er_sample * container.current_task.index)
    ...
    def autoencoder_training(self, container):
        ...
        autoencoder_opt = Adam(itertools.chain(self.decoder.parameters(), self.ae_conditioner.parameters(),
                                               self.projector.parameters()),
                               lr=1e-3)
        ...
        for e in ae_bar:
            if self.plot_dir is not None and (e % self.plot_step == 0 or e + 1 == self.autoencoder_epochs):
                if x_plot is not None:
                    if not first_plotted:
                        f = torchvision.utils.make_grid(x_plot.cpu(), scale_each=True, range=(0, 1)).numpy()
            ...
            for i, (_, images, labels) in enumerate(container.current_task(self.ae_batch_size)):
                ...
                if container.current_task.index > 0 and self.reg_type != 'alternation':
                    with torch.no_grad():
                        old_images, old_labels, _, _ = self.get_sampled_images(images.size(0))
    ...
    def nf_training(self, container: Container):
        ...
        inn_optimizer = torch.optim.Adam(itertools.chain(self.generator.parameters(), self.nf_conditioner.parameters()),
                                         lr=self.nf_lr,
        ...
        with torch.no_grad():
            for i, (_, images, y) in enumerate(container.current_task(self.ae_batch_size)):
                emb = container.encoder.flatten_cnn(images)
                emb = self.projector(emb)
                embs.append(emb)
                labels.append(y)
            ...
            if self.sampled_images_memory is not None:
                cat = torch.cat((embs, self.sampled_images_memory.tensors[2].to(self.device)), 0)
                mn = cat.mean(0)
                std = cat.std(0)
            else:
                mn = embs.mean(0)
        ...
        data_to_iter = DataLoader(TensorDataset(embs, labels), batch_size=self.nf_batch_size, shuffle=True)
        ...
        for e in nf_bar:
            ...
            if self.plot_dir is not None and e > 0 \
                    and (e % self.plot_step == 0 or (e + 1 == self.generator_epochs and not self.tolerance_training)):
            ...
            for i, (emb, labels) in enumerate(data_to_iter):
                with torch.no_grad():
                    if container.current_task.index > 0:
                        images, old_labels, _, _ = self.get_sampled_images(emb.size(0))
            ...
            nf_bar.set_postfix({'ctol': ctol, 'best_loss': best_loss, 'loss': mean_loss})
    ...
    def combine_batches(self, xa, xb, ya, yb, zero_prob: float = None):
        ...
        if zero_prob is None:
            zero_prob = 1 - len(self.old_dataset_labels) / (len(self.dataset_labels))
    ...
    def generate_images_memory(self, encoder):
        ...
        while fixed_replay > 0:
            im, lb, nfemb = self.sample(min(self.nf_batch_size, fixed_replay), labels=self.old_dataset_labels)
            cemb = encoder(im)
        ...
        images, labels, nf_embeddings, classification_embeddings = \
            torch.cat(images), torch.cat(labels), torch.cat(nf_embeddings), torch.cat(classification_embeddings)

        self.sampled_images_memory = TensorDataset(images, labels, nf_embeddings,
                                                   classification_embeddings)
    ...
    def get_sampled_images(self, size: int):
        ...
        images, labels, nf_embeddings, encoder_embeddings = self.sampled_images_memory[indices]
    ...
    def plot(self, labels=None, size=100, u=None):
        if labels is None:
        ...
        with torch.no_grad():
            ...
            x, _, _ = self.sample(size, y=y, u=None)

            f = torchvision.utils.make_grid(x.cpu(), scale_each=True, range=(0, 1)).numpy()
            f = plt.imshow(np.transpose(f, (1, 2, 0)), interpolation='nearest')
    ...
    def plot_rec(self, x, container, y=None):
        ...
        f = torchvision.utils.make_grid(x_rec.cpu(), scale_each=True, range=(0, 1)).numpy()
        f = plt.imshow(np.transpose(f, (1, 2, 0)), interpolation='nearest')
    ...
    def get_generator(self):
        ...
        gen = RNVP(n_levels=self.levels, levels_blocks=self.blocks, input_dim=self.emb_dim,
                   n_hidden=self.n_hidden, conditioning_size=cond_size, hidden_size=self.hidden_size)
